...
const folder_snippets = ( Config.Input.Snippets )
    ? join(dirname(Arguments.config),Config.Input.Snippets) : null

const folder_styles = ( Config.Input.Styles )
    ? join(dirname(Arguments.config),Config.Input.Styles) : null
...
async function build (){
    ...
    if( folder_snippets ){
        ...
        const entries = walk(folder_snippets,options);
    ...
    liquid = insertSnippet(liquid)


    function insertSnippet (
        parent : string
    ){
        return parent.replaceAll(/<!-- *Insert *(.+?) *-->/g,( _ : any , path : string ) => {
            ...
            let content = snippets.get(file) ?? ''

            content = insertSnippet(content)
    ...
    if( folder_styles ){
        ...
        const entries = walk(folder_styles,options);
...
if( Arguments.watch ){
    ...
    const watched = [ template_input ]

    if( folder_snippets )
        watched.push(folder_snippets)

    if( folder_styles )
        watched.push(folder_styles)
